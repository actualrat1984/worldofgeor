...
test('invite codes and protected route classification fail closed', () => {
  ...
  assert.equal(__test.isPrivatePath('/search-sources.js'), true)
  assert.equal(__test.isPrivatePath('/offline-sync.js'), true)
  ...
  assert.equal(__test.isPrivatePath('/notebook'), true)
  assert.equal(__test.isPrivatePath('/notebook/'), true)
  assert.equal(__test.isPrivatePath('/notebook.html'), true)
  assert.equal(__test.isPrivatePath('/notebook.js'), true)
  ...
  assert.equal(__test.isPrivatePath('/compiler.js'), true)
  assert.equal(__test.isPrivatePath('/manuscript-presence.js'), true)
  assert.equal(__test.isPrivatePath('/mentions.js'), true)
  assert.equal(__test.isPrivatePath('/inventory.js'), true)
  assert.equal(__test.isPrivatePath('/chapter-meta.js'), true)
  assert.equal(__test.isPrivatePath('/boards'), true)
  assert.equal(__test.isPrivatePath('/boards/'), true)
  assert.equal(__test.isPrivatePath('/boards.html'), true)
  assert.equal(__test.isPrivatePath('/boards.js'), true)
  ...
  assert.equal(__test.isPrivatePath('/gallery-curation.js'), true)
  ...
  assert.equal(__test.isPrivatePath('/entry'), true)
  assert.equal(__test.isPrivatePath('/entry/'), true)
  assert.equal(__test.isPrivatePath('/entry.html'), true)
  assert.equal(__test.isPrivatePath('/entry.js'), true)
  assert.equal(__test.isPrivatePath('/recaps'), true)
  assert.equal(__test.isPrivatePath('/recaps/'), true)
  assert.equal(__test.isPrivatePath('/recaps.html'), true)
  assert.equal(__test.isPrivatePath('/recaps.js'), true)
  ...
  assert.equal(__test.isPrivatePath('/world-clock.js'), true)
...
// --- Wave F4: quest board routes stay behind the member gate ----------------
test('quest board routes are private and served behind the member gate', () => {
  for (const path of ['/quests', '/quests/', '/quests.html', '/quests.js']) {
    assert.equal(__test.isPrivatePath(path), true, path)
  }
})
